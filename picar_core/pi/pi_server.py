...
class PCA9685:
  ...
  def setServoPulse(self, channel, pulse):
    if channel in self.pwrange:
        ...
        if pulse<minv: pulse = minv
        elif pulse>maxv: pulse = maxv
...
class ServoHatDriver(OutputDriver):
    ...
    def _throttle_to_us(self, throttle: float) -> int:
        ...
        if throttle >= 0: return int(round(self.esc_neutral + throttle * (self.esc_max - self.esc_neutral)))
        else: return int(round(self.esc_neutral + throttle * (self.esc_neutral - self.esc_min)))
        ...
        async def on_icecandidate(candidate):
            if candidate:
                await ws.send(json.dumps({
                    "type": "webrtc_ice",
                    "candidate": {"candidate": candidate.to_sdp(), "sdpMid": candidate.sdpMid, "sdpMLineIndex": candidate.sdpMLineIndex}
                }))
